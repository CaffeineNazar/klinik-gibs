<x-app-layout>
    ...
    <div class="py-4" x-data="{ 
        ...
        <div class="max-w-full mx-auto px-2 sm:px-4 lg:px-4">
            ...
            <div class="mb-4 flex flex-col xl:flex-row gap-3 justify-between items-center bg-white p-3 rounded-xl shadow-sm border border-gray-200">
                ...
                <div class="w-full xl:w-auto flex flex-col sm:flex-row items-center gap-2">
                    ...
                    <select x-model="tingkat" @change="filterData()" class="w-full sm:w-auto bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 p-2.5 cursor-pointer transition-colors min-w-[150px]">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden relative">
                    ...
                    <table class="min-w-full divide-y divide-gray-200 text-left border-collapse">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th scope="col" class="py-4 px-5 text-xs font-bold text-gray-500 uppercase tracking-wider">Pasien / Siswa</th>
                                ...
                                <th scope="col" class="py-4 px-5 text-xs font-bold text-gray-500 uppercase tracking-wider w-40">HRT (Wali Kelas)</th>
                        ...
                        <tbody class="divide-y divide-gray-100 bg-white" x-show="isLoading" style="display: none;">
                            <template x-for="i in 5" :key="i">
                                <tr class="animate-pulse">
                                    ...
                                    <td class="py-4 px-5 align-top">
                                        <div class="h-16 bg-gray-100 rounded-lg w-full"></div>
                                    </td>
                                    <td class="py-4 px-5 align-top">
                        ...
                        <tbody class="divide-y divide-gray-100 bg-white" x-show="!isLoading">
                            ...
                            <tr class="hover:bg-slate-50 transition-colors duration-150 group">
                                <td class="py-4 px-5 align-top whitespace-nowrap">
                                    ...
                                    $jamInt = $waktu->hour;
                                    if ($jamInt >= 5 && $jamInt < 12) { $periode='Morning' ; $badgeColor='bg-amber-100 text-amber-700 border-amber-200' ; $icon='M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z' ; }
                                        elseif ($jamInt>= 12 && $jamInt < 18) { $periode='Afternoon' ; $badgeColor='bg-orange-100 text-orange-700 border-orange-200' ; $icon='M3 15h18M9 21V3m6 18V3M4.22 4.22l15.56 15.56M4.22 19.78L19.78 4.22' ; }
                                            else { $periode='Evening' ; $badgeColor='bg-indigo-100 text-indigo-700 border-indigo-200' ; $icon='M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z' ; }
                                            @endphp
                                            <div class="flex flex-col items-start">
                                            <span class="text-sm font-bold text-gray-900 mb-1">{{ $waktu->translatedFormat('d F Y') }}</span>
                                            <div class="flex items-center text-gray-500 text-xs font-medium mb-2">
                                                <svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                                </svg>
                                                {{ $waktu->format('H:i') }} WITA
                                            </div>
                ...
                <td class="py-4 px-5 align-top">
                    <div class="flex items-start">
                        <div class="flex-shrink-0 h-10 w-10 rounded-full bg-blue-100 border border-blue-200 flex items-center justify-center text-blue-700 font-bold text-lg mr-3 shadow-sm">{{ strtoupper(substr($r->nama_siswa, 0, 1)) }}</div>
                        <div class="flex flex-col">
                            ...
                            <div class="mt-2 flex flex-col items-start gap-1.5">
                                <span class="bg-gray-100 text-gray-600 border border-gray-200 px-2 py-0.5 rounded-md text-xs font-semibold">Kelas {{ $r->nama_kelas ?? 'Tanpa Kelas' }}</span>
                            </div>
                        </div>
                    </div>
                </td>
                ...
                <td class="py-4 px-5 align-top">
                    <div class="space-y-2">
                        ...
                        <div class="inline-flex items-center text-xs font-medium text-gray-400 bg-gray-50 py-1 px-2 rounded-md border border-gray-200"><svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path>
                            </svg>Belum ada detail</div>
                        @endif
                    </div>
                </td>
                ...
                <td class="py-4 px-5 align-middle text-center">
                    <div class="flex flex-col space-y-2">
                        ...
                        <div class="w-full bg-blue-50 text-blue-600 text-[11px] font-bold py-2.5 px-3 rounded-lg border border-blue-200 flex items-center justify-center gap-1.5 shadow-sm">
                        ...
                        @endif
                        @else
                        <div class="w-full bg-gray-100 text-gray-400 text-[10px] font-bold py-2.5 px-3 rounded-lg border border-gray-200 cursor-not-allowed uppercase tracking-wider flex items-center justify-center gap-1.5" title="Hanya bisa mengubah status di hari yang sama">
                        ...
                        <button type="button" @click="editData = { id: '{{ $r->id_rekam_medis }}', keluhan: {{ json_encode($r->keluhan ?? '') }}, diagnosa: {{ json_encode($r->diagnosa ?? '') }}, tindakan: {{ json_encode($r->tindakan ?? '') }} }; isEditModalOpen = true;" class="w-full bg-white text-blue-600 border border-blue-200 hover:bg-blue-50 hover:border-blue-300 text-xs font-semibold py-2 px-3 rounded-lg shadow-sm transition-all focus:ring-2 focus:ring-blue-100 focus:outline-none inline-flex items-center justify-center gap-1.5">
                ...
                <tr>
                    <td colspan="6" class="py-16 px-4 text-center">
                        <div class="flex flex-col items-center justify-center">
                            <div class="bg-gray-50 rounded-full p-4 mb-4"><svg class="h-12 w-12 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="mt-4" x-show="!isLoading">{{ $riwayats->links() }}</div>
    </div>
    ...
    <div x-show="isEditModalOpen" style="display: none;" class="fixed inset-0 z-[9999] overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div x-show="isEditModalOpen" @click="isEditModalOpen = false" x-transition.opacity class="fixed inset-0 bg-gray-900 bg-opacity-50 backdrop-blur-sm transition-opacity"></div>
            ...
            <div x-show="isEditModalOpen" x-transition class="inline-block align-bottom bg-white rounded-xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-xl sm:w-full relative">
                <form :action="`/rekam-medis/${editData.id}`" method="POST">
                    ...
                    <div class="bg-white px-6 pt-6 pb-6">
                        <h3 class="text-xl font-bold text-gray-900 border-b border-gray-100 pb-3 mb-5 flex items-center"><svg class="w-6 h-6 mr-2 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                            </svg>Edit Rekam Medis</h3>
                        <div class="space-y-4">
                            <div><label class="block text-gray-700 text-sm font-bold mb-1.5">Keluhan (Wajib)</label><textarea x-model="editData.keluhan" name="keluhan" rows="3" class="shadow-sm border-gray-300 rounded-lg w-full py-2.5 px-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-gray-50 text-gray-900" required></textarea></div>
                            <div><label class="block text-gray-700 text-sm font-bold mb-1.5">Diagnosa <span class="text-gray-400 font-normal ml-1">(Opsional)</span></label><input type="text" x-model="editData.diagnosa" name="diagnosa" class="shadow-sm border-gray-300 rounded-lg w-full py-2.5 px-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-gray-50 text-gray-900" placeholder="Contoh: Gejala Magh..."></div>
                            <div><label class="block text-gray-700 text-sm font-bold mb-1.5">Tindakan <span class="text-gray-400 font-normal ml-1">(Opsional)</span></label><input type="text" x-model="editData.tindakan" name="tindakan" class="shadow-sm border-gray-300 rounded-lg w-full py-2.5 px-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-gray-50 text-gray-900" placeholder="Contoh: Diberikan Promag..."></div>
                        </div>
                    </div>
                    <div class="bg-gray-50 px-6 py-4 sm:flex sm:flex-row-reverse border-t border-gray-100 gap-3">
                        <button type="submit" class="w-full inline-flex justify-center rounded-lg border border-transparent shadow-sm px-5 py-2.5 bg-blue-600 text-sm font-bold text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:w-auto transition-colors">Simpan Perubahan</button>
